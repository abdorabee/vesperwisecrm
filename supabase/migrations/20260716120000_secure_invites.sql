-- Server-issued invites. Membership on signup is granted only when a
-- matching unredeemed invite row exists for the new user's email.
--
-- Previously handle_new_user() trusted raw_user_meta_data
-- (invited_account_id / invited_role), which is client-settable on
-- supabase.auth.signUp with the anon key -- anyone who knew an account
-- UUID could self-provision as an admin of that tenant. Invite actions
-- now write an invites row first, and the trigger honors only that.
...
create table public.invites (
  ...
  -- client invites must carry a client_id; staff invites must not
  constraint invites_client_role_pairing check (
...
create index invites_account_idx on public.invites (account_id);

alter table public.invites enable row level security;

create policy "admin select own account invites" on public.invites
  for select using (public.is_account_admin(account_id));
create policy "admin insert own account invites" on public.invites
  for insert with check (public.is_account_admin(account_id));
create policy "admin delete own account invites" on public.invites
...
begin
  -- Membership via invite is granted only from a server-issued invites
  -- row. raw_user_meta_data is client-controllable and must never decide
  -- account membership.
  ...
  if found then
    ...
    values (
      ...
      new.id,
      pending_invite.role,
      case when pending_invite.role = 'client' then pending_invite.client_id else null end
    )
