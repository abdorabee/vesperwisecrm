-- Seed opinionated defaults on account creation instead of a blank slate:
-- starter tags, two starter sequences, and two flagging workflows built on
-- the existing engine (no new trigger/action types).
...
begin
  ...
  -- Starter tags, referenced by the flagging workflows below.
  ...
  -- Flagging workflows so a leaking pipeline shows up without any setup.
